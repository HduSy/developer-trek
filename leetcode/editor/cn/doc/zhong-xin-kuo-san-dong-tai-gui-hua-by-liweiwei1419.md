...
+ 时间复杂度：*O(N^2)*，这里 *N* 是原始字符串的长度。新字符串的长度是 ![2\timesN+1 ](./p__2_times_N+_1_.png) ，不计系数与常数项，因此时间复杂度仍为 *O(N^2)*。
+ 空间复杂度：*O(N)*。
...
![center=argmax\{x+p\[x\]\;|\;0\leqx<i\} ](./p___center_=_argmax_{x_+_p_x___;_|_;__0_leq_x__i_}__.png) 
